...
// backend/modules/cheques/echeq_cartera/echeq_cartera.php
...
try {
  $limit  = echeq_cartera_as_int($_GET['limit'] ?? 100, 100);
  $offset = echeq_cartera_as_int($_GET['offset'] ?? 0, 0);
  ...
  $sql = "
    SELECT
      ...
      COALESCE(ca_directo.archivo_path, ca_rel.archivo_path, '') AS archivo_path,
      COALESCE(ca_directo.archivo_url, ca_rel.archivo_url, '') AS archivo_url,
    ...
    LEFT JOIN comprobantes_archivos ca_rel
      ON ca_rel.id_movimiento = mc.id_movimiento
    ...
    GROUP BY
      ...
      ca_directo.archivo_path,
      ca_directo.archivo_url,
  ...
  $echeqs = array_map(static function (array $row): array {
    $archivoPath = (string)($row['archivo_path'] ?? '');
    $archivoUrl  = (string)($row['archivo_url'] ?? '');
    $idComp      = isset($row['id_comprobante']) ? (int)$row['id_comprobante'] : 0;

    $tieneComprobante = (
      !empty($row['tiene_comprobante']) &&
      $idComp > 0 &&
      (trim($archivoPath) !== '' || trim($archivoUrl) !== '')
    );
  ...
  echeq_cartera_fail(
    'Error al listar echeqs en cartera: ' . $e->getMessage(),
    500,
    [
      'debug' => [
        'archivo' => $e->getFile(),
