...
function describeLineProblem(r, idx1based) {
  ...
  if (!(Number.isFinite(detId) && detId > 0)) {
    issues.push(detTxt ? `el detalle "${detTxt}" no está seleccionado` : "falta el detalle");
  }
...
function getAuthInfo() {
  ...
  try {
    ...
    const c = u?.idUsuario ?? u?.id_usuario ?? u?.id ?? u?.user_id ?? 0;
...
export default function ModalNuevaCompra({ open, lists, onClose, onToast, onSaved }) {
  ...
  const [compBlobUrl, setCompBlobUrl] = useState("");
  const compBlobRef = useRef("");

  useEffect(() => {
    if (compBlobRef.current) {
      URL.revokeObjectURL(compBlobRef.current);
      compBlobRef.current = "";
    }

    if (archivoAdjunto) {
      const url = URL.createObjectURL(archivoAdjunto);
      compBlobRef.current = url;
      setCompBlobUrl(url);
    } else {
      setCompBlobUrl("");
    }
  ...
  const removeMedioPago = useCallback((id) => {
    setMediosFilas((prev) => {
      ...
      if (nuevas.length === 0) return [buildEmptyMedioPago()];
      return nuevas;
    });
  }, []);
  ...
  const validate = useCallback(() => {
    ...
    if (isContado) {
      ...
      if (sumaMediosPago < resumen.total - 0.05 && resumen.total > 0) {
        return {
          ok: false,
          msg: `La suma de los medios de pago (${moneyARS(sumaMediosPago)}) no cubre el total de la compra (${moneyARS(resumen.total)}).`,
        };
      }
    }
  ...
  const handleOpenVerComprobante = useCallback(() => {
    if (!archivoAdjunto || !compBlobUrl) return;
    setOpenVerComp(true);
  }, [archivoAdjunto, compBlobUrl]);
