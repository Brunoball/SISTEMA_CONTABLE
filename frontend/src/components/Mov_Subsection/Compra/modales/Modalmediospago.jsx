...
/* =========================================================
   CHEQUE CARD — diseño inspirado en cheque bancario
   ========================================================= */
        ...
        <div className="cheque-card__brand">
          <div className="cheque-card__logo-icon">
            <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
              <path d="M8 2C5.8 2 4 3.6 4 5.6c0 1.4.8 2.6 2 3.2V13h4V8.8c1.2-.6 2-1.8 2-3.2C12 3.6 10.2 2 8 2z" fill="currentColor" opacity="0.9"/>
            </svg>
          </div>
          <div>
            <div className="cheque-card__bank-name">{safeText(ch?.banco) !== "-" ? safeText(ch?.banco) : "Cheque"}</div>
            <div className="cheque-card__bank-sub">{isEcheq ? "E-CHEQ ELECTRÓNICO" : "CHEQUE DE PAGO DIFERIDO"}</div>
          </div>
        </div>
        <div className="cheque-card__header-right">
          <span className="cheque-card__num-label">N°</span>
          <span className="cheque-card__num-value">{safeText(ch?.numero_cheque)}</span>
        </div>
      ...
      <div className="cheque-card__body">
        <div className="cheque-card__row cheque-card__row--spaced">
          <div className="cheque-card__field cheque-card__field--wide">
            ...
            <div className="cheque-card__field-line">{safeText(ch?.emisor)}</div>
          </div>
          <div className="cheque-card__field">
            <span className="cheque-card__field-label">F. emisión</span>
            <div className="cheque-card__field-line cheque-card__field-line--mono">{safeText(formatFechaDMY(ch?.fecha_emision))}</div>
          </div>
        </div>
        <div className="cheque-card__row cheque-card__row--spaced">
          <div className="cheque-card__field">
            <span className="cheque-card__field-label">F. pago</span>
            <div className="cheque-card__field-line cheque-card__field-line--mono">{safeText(formatFechaDMY(ch?.fecha_pago))}</div>
          </div>
          <div className="cheque-card__importe-box">
            ...
            <span className="cheque-card__importe-value">
              {ch?.importe > 0 ? moneyARS(ch.importe) : <span className="cheque-card__field-empty">0,00</span>}
            </span>
          </div>
        </div>
      </div>
    ...
    <div className="cheques-cards-pn">
      {cheques.map((ch, idx) => (
        <ChequeCard
          ...
          ch={ch}
          checked={idsSeleccionados.includes(String(ch?.id_cheque))}
          onToggle={onToggle}
        />
      ))}
    </div>
...
/* ── Fila de un medio de pago ── */
function MpRow({ row, idx, mediosPagoList, totalCompra, sumaMediosPago, onUpdate, onRemove, apiGet, BASE_URL, showToast }) {
    ...
    <div className="mp-card">
      <div className="mp-card__top">
        ...
        <div>
          ...
          <select className="mp-select" value={String(row.id_medio_pago || "")} onChange={e => handleChangeMedio(e.target.value)}>
        ...
        <div className="mp-card__actions">
          {!esCheque && (
            <button type="button" className="mp-btn-completar"
              onClick={() => onUpdate(row.id, { monto: restanteParaEstaFila, montoDraft: "", montoFocused: false })}
              disabled={!puedeCompletarRestante} title="Completar importe restante">↓ Rest.</button>
          )}
          <button type="button" className="mp-btn-del" onClick={() => onRemove(row.id)} title="Quitar">×</button>
        </div>
      </div>
      ...
      {esCheque && (
        <div className="mp-cheques-panel">
          ...
          {row.loadingCheques ? (
            <div className="mp-cheques-loading"><FontAwesomeIcon icon={faCircleNotch} spin /> Cargando…</div>
...
/* ============================================================
   MODAL PRINCIPAL MEDIOS DE PAGO
   ============================================================ */
        ...
        <div className="mp-modal__body">
          {mediosFilas.map((mp, idx) => (
            <MpRow
              ...
              apiGet={apiGet} BASE_URL={BASE_URL} showToast={showToast}
            />
          ))}
        </div>
    ...
    <div className="nc-pago-resumen">
      ...
      <div className="nc-pago-resumen__body">
        {filasConMedio.map(mp => {
          ...
          const esCheque = tipoCheque !== null;
          const cantCheques = Array.isArray(mp.id_cheque) ? mp.id_cheque.length : mp.id_cheque ? 1 : 0;
          return (
            <div key={mp.id} className="nc-pago-resumen__row">
              <div className="nc-pago-resumen__medio">
                <div className="nc-pago-resumen__dot"/>
                <span className="nc-pago-resumen__nombre" title={nombre}>{nombre}</span>
                {esCheque && cantCheques > 0 && (
                  <span className="nc-pago-resumen__cheque-badge">{cantCheques} {tipoCheque === "echeq" ? "eCheq" : "ch."}</span>
                )}
              </div>
