...
function formatFechaDMY(v) {
  ...
  if (m) {
    return `${String(Number(m[3])).padStart(2, "0")}/${String(Number(m[2])).padStart(
      2,
      "0"
    )}/${m[1]}`;
  }
    ...
    <div className={["cheque-card", isEcheq ? "cheque-card--echeq" : ""].filter(Boolean).join(" ")}>
      <div className="cheque-card__header">
        ...
        <div className="cheque-card__brand">
          ...
          <div>
            <div className="cheque-card__bank-name">{safeText(item?.medio_pago_nombre)}</div>
        ...
        <div className="cheque-card__header-right">
          ...
          <span className="cheque-card__num-value">{safeText(item?.numero_cheque)}</span>
        </div>
      ...
      <div className="cheque-card__body">
        <div className="cheque-card__row cheque-card__row--spaced">
          <div className="cheque-card__field cheque-card__field--wide">
            ...
            <div className="cheque-card__field-line">{safeText(item?.emisor)}</div>
          </div>
  ...
  return (
    <div className="mdp-medio-card">
      ...
      <div className="mdp-medio-card__info">
        <span className="mdp-medio-card__nombre">{safeText(item?.medio_pago_nombre)}</span>
        <span className="mdp-medio-card__sub">Medio de pago</span>
      </div>

      <div className="mdp-medio-card__monto">{moneyARS(item?.monto || 0)}</div>
    </div>
  );
}
...
/* =========================
   Modal principal
========================= */
export default function ModalDetalleMediosPagoEgreso({ open, row, onClose }) {
  ...
  const detalleTexto =
    safeText(row?.detalle ?? row?.descripcion ?? row?.concepto) === "—"
      ? ""
      : safeText(row?.detalle ?? row?.descripcion ?? row?.concepto);

  return createPortal(
    <div className="mi-modal__overlay" onMouseDown={onClose}>
      <div className="mi-modal__container mdp-container" onMouseDown={(e) => e.stopPropagation()}>
        <div className="mi-modal__header">
          ...
          <div className="mi-modal__head-left">
            ...
            >
              Egreso #{safeText(row?.id_movimiento)}
        ...
        <div className="mi-modal__content mdp-content">
          {detalle.length === 0 ? (
            <div className="mdp-empty">
              ...
              <span>Este egreso no tiene detalle de medios de pago guardado.</span>
            </div>
          ) : (
            <>
              <div className="mdp-items-grid">
                {detalle.map((item, index) => {
                  ...
                  const key =
                    ...
                    `${item?.id_medio_pago || "mp"}-${item?.id_cheque || "x"}-${index}`;
