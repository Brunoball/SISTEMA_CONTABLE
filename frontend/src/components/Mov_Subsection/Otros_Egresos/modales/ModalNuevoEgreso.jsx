...
function getDetalleId(d) {
  const c=d?.id??d?.id_detalle??d?.idDetalle??d?.detalle_id??d?.id_categoria_egreso??d?.idCategoriaEgreso??d?.id_stock_producto??null;
  const n=Number(c); return Number.isFinite(n)&&n>0?n:null;
}
...
function getSavedMovimientoIdFromResponse(data,init=null) {
  for(const c of [data?.id_movimiento,data?.movimiento_id,data?.id,data?.egreso?.id_movimiento,data?.egreso?.id,init?.id_movimiento,init?.id]){
...
function normalizeLists(lists) {
  ...
  const mediosPago=pick("medios_pago").length?pick("medios_pago"):pick("mediosPago").length?pick("mediosPago"):pick("medios").length?pick("medios"):[];
  const detallesBase=pick("detalles").length?pick("detalles"):pick("categorias_egreso").length?pick("categorias_egreso"):pick("categoriasEgreso").length?pick("categoriasEgreso"):pick("categorias").length?pick("categorias"):[];
  const detallesEgresos=pick("detalles_egresos").length?pick("detalles_egresos"):pick("detallesEgresos").length?pick("detallesEgresos"):pick("detalles_egreso").length?pick("detalles_egreso"):detallesBase;
  const clasificaciones=pick("clasificaciones").length?pick("clasificaciones"):pick("clasificacion").length?pick("clasificacion"):[];
  return {
...
function resolveClasificacionesConfig(arr=[]) {
  const parsed=arr.map(x=>({id:getClasificacionId(x),nombre:normalizeName(optionLabel(x))})).filter(x=>Number.isFinite(Number(x.id))&&Number(x.id)>0);
  const fijo=parsed.find(x=>x.nombre==="COSTO FIJO")||parsed.find(x=>x.nombre.includes("COSTO FIJO"))||null;
  const noFijo=parsed.find(x=>x.id!==fijo?.id&&(x.nombre==="COSTO VARIABLE"||x.nombre.includes("VARIABLE")||x.nombre.includes("NO ES COSTO FIJO")))||parsed.find(x=>x.id!==fijo?.id)||null;
  return {
...
async function apiGet(url)          { const r=await fetch(url,{method:"GET",headers:buildAuthHeaders(false)}); return await parseJsonOrThrow(r); }
async function apiPostForm(url,fd)  { const r=await fetch(url,{method:"POST",headers:buildAuthHeaders(false),body:fd}); return await parseJsonOrThrow(r); }
...
function buildRowFromData(r) {
  return { id:uid(), id_detalle:String(getDetalleId(r)??""), detalle:safeStr(r?.detalle??r?.descripcion??r?.concepto), cantidad:Math.max(1,safeNumber(r?.cantidad||1)), precio:safeNumber(r?.precio??r?.importe??r?.monto??0), precioDraft:"", precioFocused:false, ivaPct:safeNumber(r?.iva_pct??r?.ivaPct??0), stock_disponible:null, sinStock:false };
}
function buildRowsFromInitial(data) {
  const items=Array.isArray(data?.items)&&data.items.length?data.items:Array.isArray(data?.detalles)&&data.detalles.length?data.detalles:null;
  if(items?.length) return items.map(x=>({ id:uid(), id_detalle:String(getDetalleId(x)??""), detalle:safeStr(x?.detalle??x?.descripcion??x?.concepto??x?.detalle_nombre??""), cantidad:Math.max(1,safeNumber(x?.cantidad||1)), precio:safeNumber(x?.precio??x?.importe??x?.monto??0), precioDraft:"", precioFocused:false, ivaPct:safeNumber(x?.iva_pct??x?.ivaPct??0), stock_disponible:null, sinStock:false }));
  return [buildRowFromData(data)];
}
function describeLineProblem(r,idx) {
  const detalle=safeStr(r.detalle), qty=safeNumber(r.cantidad), price=safeNumber(r.precio), total=safeNumber(r.total);
...
function buildMediosPagoFromInitial(data) {
  ...
  if(detalle.length) {
    const rows=[]; let currentChequeRow=null;
    detalle.forEach(mp=>{
      const idMedio=String(mp?.id_medio_pago??""), idCheque=Number(mp?.id_cheque??0), chequeTipo=safeStr(mp?.cheque_tipo).toLowerCase();
      if(idCheque>0) {
        const canMerge=currentChequeRow&&String(currentChequeRow.id_medio_pago)===idMedio&&String(currentChequeRow._chequeTipo||"")===chequeTipo;
        if(!canMerge) { currentChequeRow={...buildEmptyMedioPago(),id_medio_pago:idMedio,monto:0,id_cheque:[],chequesDisponibles:[],_chequeTipo:chequeTipo}; rows.push(currentChequeRow); }
        currentChequeRow.id_cheque.push(String(idCheque));
        currentChequeRow.monto+=safeNumber(mp?.monto??mp?.cheque_importe??0);
        currentChequeRow.chequesDisponibles.push({ id_cheque:idCheque, tipo:chequeTipo, emisor:safeStr(mp?.emisor), numero_cheque:safeStr(mp?.numero_cheque), fecha_emision:safeStr(mp?.fecha_emision), fecha_pago:safeStr(mp?.fecha_pago), importe:safeNumber(mp?.cheque_importe??mp?.monto) });
      } else {
  ...
  const legacyId=Number(data?.id_medio_pago??data?.medio_pago_id??0), legacyMonto=safeNumber(data?.monto_total??data?.total??0);
...
/** Fila de medio de pago — usa clases mp-* del CSS de NuevaCompra */
function MedioPagoRow({ row, mediosPagoList, totalEgreso, sumaMediosPago, onUpdate, onRemove, saving, showToast }) {
  ...
  const handleChangeMedio=useCallback(async(val)=>{
    ...
    if(tipo!==null) {
      try {
        const sp=new URLSearchParams(); sp.set("action","otros_egresos_cheques_cartera_listar"); sp.set("tipo",tipo);
  ...
  const handleToggleCheque=useCallback(idStr=>{
        ...
        <div className="nc-mp-medio">
          ...
          >
            ...
            {mediosPagoList.map(x=>{
              ...
              return <option key={idMp??uid()} value={idMp!=null?String(idMp):""}>{String(x?.nombre??"").trim()||"Medio"}</option>;
            })}
          </select>
        </div>
...
<div className="nc-mp-cheques">
  <div className="nc-mp-cheques-title">
          ...
          {row.loadingCheques?(
            <div className="mp-cheques-loading"><FontAwesomeIcon icon={faCircleNotch} spin style={{marginRight:6}}/>Cargando...</div>
...
export default function ModalNuevoEgreso({ open, mode="create", initialData=null, lists, onClose, onToast, onSubmit, onSaved }) {
  ...
  useEffect(()=>{
    ...
    if(!wasOpen&&open) {
      ...
      setMediosFilas(isEdit&&(getMovimientoId(initialData)||initialData)?buildMediosPagoFromInitial(initialData):[buildEmptyMedioPago()]);
  ...
  /* Nueva descripción */
  const handleCrearNuevaDescripcion=useCallback((rowId)=>{ setCurrentRowIdForNewDesc(rowId); setOpenNuevaDescModal(true); },[]);
  ...
  const handleGuardarNuevaDescripcion=useCallback(async(nombreDescripcion)=>{
    try {
      ...
      if(sessionKey) h["X-Session"]=sessionKey;
      if(token) h.Authorization=`Bearer ${token}`;
      const response=await fetch(API_DETALLES_CREAR,{method:"POST",headers:h,body:JSON.stringify({nombre:nombreDescripcion,idUsuario})});
      ...
      showToast("error",error.message||"No se pudo crear la descripción.",3000); return false;
    }
  ...
  const handleCantidadChange=useCallback((rowId,newCantidad)=>{
    ...
    if(row.stock_disponible!==null&&row.stock_disponible!==undefined&&row.stock_disponible!==""&&typeof cantidadFinal==="number"&&cantidadFinal>Number(row.stock_disponible)) {
      cantidadFinal=Number(row.stock_disponible); showToast("advertencia",`Stock máximo disponible: ${row.stock_disponible}`,2000);
    }
  ...
  const rowsCalc=useMemo(()=>rows.map(r=>{
    const cantidad=Math.max(0,safeNumber(r.cantidad)), precio=Math.max(0,safeNumber(r.precio)), ivaPct=Math.max(0,safeNumber(r.ivaPct));
    const subtotal=cantidad*precio, ivaMonto=subtotal*(ivaPct/100), total=subtotal+ivaMonto;
  ...
  const validate=useCallback(()=>{
    ...
    if(sumaMediosPago<resumen.total-0.05&&resumen.total>0) return { ok:false, msg:`La suma de los medios de pago (${moneyARS(sumaMediosPago)}) no cubre el total del egreso (${moneyARS(resumen.total)}).` };
    const problems=[];
  ...
  const buildPayload=useCallback(()=>{
    ...
    const mediosPagoPayload=mediosFilas.flatMap(mp=>{
      ...
      if(tipoCheque!==null&&chequesSeleccionados.length>0) {
        return chequesSeleccionados.map(idChequeStr=>{ const ch=mp.chequesDisponibles.find(x=>String(x.id_cheque)===idChequeStr); return { id_medio_pago:Number(mp.id_medio_pago), monto:Number(ch?.importe||0), id_cheque:Number(idChequeStr), cheque_tipo:tipoCheque }; });
      }
      return [{ id_medio_pago:Number(mp.id_medio_pago), monto:safeNumber(mp.monto) }];
    });
    const primerMedio=mediosPagoPayload[0]||null;
    const medioLegacy=primerMedio&&Number(primerMedio.id_medio_pago)>0?mediosPagoList.find(x=>Number(getMedioPagoId(x))===Number(primerMedio.id_medio_pago)):null;
    return {
      ...
      subtotal:safeNumber(subtotalFinal), iva_monto:safeNumber(ivaFinal),
      monto_total:safeNumber(totalFinal), total:safeNumber(totalFinal), total_general:safeNumber(totalFinal),
      items:usableRows.map((x,idx)=>({ orden:idx+1, id_detalle:Number(x.id_detalle||0)||null, id_stock_producto:null, detalle:safeStr(x.detalle), descripcion:safeStr(x.detalle), concepto:safeStr(x.detalle), cantidad:safeNumber(x.cantidad), precio:safeNumber(x.precio), iva_pct:safeNumber(x.ivaPct), subtotal:safeNumber(x.subtotal), iva_monto:safeNumber(x.ivaMonto), total:safeNumber(x.total) })),
    };
  ...
  const subirArchivo=useCallback(async(idMovimiento,archivo)=>{
    ...
    const fd=new FormData(); fd.append("archivo",archivo); fd.append("tipo","OTRO_EGRESO"); fd.append("id_movimiento",String(idMovimiento)); fd.append("force_replace","1");
                      ...
                      <div key={r.id} className={`mi-cr-row${rowSinStock?" mi-cr-row--sin-stock":""}`}>
                        <div className="mi-cr-cell mi-cr-cell--detalle">
                          <GlobalAutocomplete
                            ...
                            getOptionLabel={d=>{ if(d&&d.__isNewOption) return d.nombre; return optionLabel(d); }}
                            getOptionValue={d=>{ if(d&&d.__isNewOption) return "__new_option__"; return String(getDetalleId(d)??optionLabel(d)); }}
  ...
  {diferenciaRestante <= 0.01 && sumaMediosPago > 0 && (
    <span className="nc-mp-totals-ok">✓ Cubierto</span>
                  ...
                  <div className="nc-section-body">
                    <div className="mi-uploadCard">
                      <div className="mi-uploadCard__head">
                        ...
                        <div className="mi-uploadCard__sub">Seleccioná o quitá el archivo antes de guardar</div>
                      </div>
                      <div className="mi-uploadCard__body">
                        ...
                        <div className="mi-uploadBar" style={{marginTop:10}}>
                          ...
                          <button type="button" className="mi-uploadBar__btn mi-uploadBar__btn--primary" onClick={()=>{ if(!saving) fileInputRef.current?.click(); }} disabled={saving}>
                            <FontAwesomeIcon icon={faFileInvoiceDollar}/>{" "}{archivoAdjunto?"Reemplazar archivo":"Seleccionar archivo"}
                          </button>
                        </div>
                      </div>
                    </div>
                ...
                <div className="nc-actions mi-cr-filters__actions">
                  <button type="button" className="mit-btn mit-btn--solid mit-btn--block" onClick={submit} disabled={saving}>{btnLabel}</button>
