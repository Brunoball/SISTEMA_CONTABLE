...
import { PanelMediosPagoInlineVenta } from "./PanelMediosPagoIngresoLocal.jsx";
      ...
      Number(mp?.id_cheque ?? 0) > 0
        ? {
            ...
            tipo_cheque: safeText(
              mp?.cheque_tipo || detectChequeTipoFromMedio(mp?.medio_pago_nombre || "") || "cheque"
            ),
...
function MedioPagoIngresoRow({
  ...
  const tipoCheque = useMemo(() => detectChequeTipoFromMedio(medio?.nombre || ""), [medio]);
    ...
    <div className="nc-mp-card">
      <div className="nc-mp-inline">
        ...
        <div className="nc-mp-monto-wrap">
          ...
          <input
            className={`nc-mp-input-monto ${esCheque && row.chequeData ? "nc-mp-input-monto--locked" : ""}`}
            type="text"
            inputMode="decimal"
            ...
            placeholder="$ 0,00"
            disabled={saving || (esCheque && !!row.chequeData)}
          />
        </div>
        ...
        <div className="nc-mp-actions-col">
          ...
          <button type="button" className="nc-mp-del-btn" onClick={() => onRemove(row)} disabled={saving}>
            ×
          </button>
        </div>
      </div>
      ...
      {esCheque && (
        <div className="nc-mp-cheques">
          <div className="nc-mp-cheques-title">
            <FontAwesomeIcon icon={faMoneyCheckDollar} /> {tipoCheque === "echeq" ? "eCheq" : "Cheque"}
          </div>
          ...
          {row.chequeData ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              >
                ...
                <div>
                  <b>Importe:</b> {moneyARS(row.chequeData.importe)}
                </div>
                <div>
              ...
              <button
                ...
                onClick={() => onOpenCheque(row.id, tipoCheque)}
                disabled={saving}
              >
...
function mapIngresoMedioToVentaRow(mp) {
  return {
    ...mp,
    cheque: mp?.cheque ?? mp?.chequeData ?? null,
  };
}

function adaptVentaPatchToIngreso(patch) {
  if (!patch || typeof patch !== "object") return patch;
  if (Object.prototype.hasOwnProperty.call(patch, "cheque")) {
    return { ...patch, chequeData: patch.cheque, cheque: patch.cheque };
  }
  return patch;
}
...
export default function ModalEditarIngreso({
  ...
  useEffect(() => {
    ...
    setRemovedChequeIds([]);
    setChequeEditor({ open: false, medioRowId: null, tipoCheque: "cheque" });
  ...
  useEffect(() => {
    ...
    const h = (e) => {
      ...
      if (openViewer || chequeEditor.open || openNuevaDescripcionModal) return;
      e.preventDefault();
      e.stopPropagation();
      onClose?.();
    };
  ...
  }, [open, saving, openViewer, chequeEditor.open, openNuevaDescripcionModal, onClose]);
  ...
  const cargarInfoComprobante = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API}?action=otros_ingresos_comprobantes_info&id_movimiento=${idMovimiento}`, {
        method: "GET",
  ...
  const removeItem = useCallback(
    (uid) =>
      setForm((p) =>
        (p.items || []).length <= 1 ? p : { ...p, items: p.items.filter((it) => it.uid !== uid) }
      ),
    []
  );
  ...
  const guardarCheque = useCallback(
    async (idMovimiento, medioDetalle, chequeData) => {
      ...
      if (chequeData.archivo instanceof File) {
        fd.append("archivo", chequeData.archivo, chequeData.archivo_nombre || chequeData.archivo.name || "adjunto");
      }
      ...
      const action = chequeData.id_cheque ? "otros_ingresos_cheques_actualizar" : "otros_ingresos_cheques_guardar";
      if (chequeData.id_cheque) fd.append("id_cheque", String(Number(chequeData.id_cheque)));

      const res = await fetch(`${API}?action=${action}`, { method: "POST", headers, body: fd });
      const data = await parseJsonOrThrow(res);
  ...
  const validate = useCallback(() => {
    ...
    for (let i = 0; i < form.medios.length; i++) {
      ...
      if (tipoCheque) {
        if (!mp.chequeData) {
          return { ok: false, msg: `Medio de pago ${i + 1}: debés cargar el ${tipoCheque === "echeq" ? "eCheq" : "cheque"}.` };
        }
    ...
    try {
      ...
      const payload = {
        ...
        medios_pago: (form.medios || []).map((mp) => ({
          ...
          monto: safeNumber(mp.chequeData?.importe ?? mp.monto),
          id_cheque: mp.id_cheque || null,
          cheque_tipo: mp.chequeData?.tipo_cheque || null,
        })),
        items,
      };
      ...
      const mediosDetalle = Array.isArray(resp?.medios_pago_detalle) ? resp.medios_pago_detalle : [];
      for (let index = 0; index < (form.medios || []).length; index++) {
        const mp = form.medios[index];
        if (!mp.chequeData) continue;

        const detalleMp =
          mediosDetalle.find(
            (x) => Number(x?.id_movimiento_medio_pago || 0) === Number(mp.id_movimiento_medio_pago || 0)
          ) ||
          mediosDetalle[index] ||
          null;

        await guardarCheque(idMovimientoFinal, detalleMp, mp.chequeData);
      }

      for (const idCheque of removedChequeIds) {
        try {
          await eliminarCheque(idCheque);
        } catch {}
      }
  ...
  return createPortal(
    <>
        ...
        >
          <div className="mi-modal__header">
            ...
            <div className="mi-modal__head-left">
              ...
              <p className="mi-modal__subtitle">Modificá fecha, medios de pago, ítems y comprobante</p>
            </div>
                ...
                <section className="mi-cr-table">
                  ...
                  <div className="mi-cr-table__foot">
                    <div className="mi-cr-foot-actions">
                      <button type="button" className="nv-foot-btn" onClick={addItem} disabled={saving}>
                    ...
                    <div className="nc-section">
                      ...
                      <div className="nc-section-body">
                        ...
                        <PanelMediosPagoInlineVenta
                          mediosFilas={(form.medios || []).map(mapIngresoMedioToVentaRow)}
                          mediosPagoList={mediosPago}
                          totalCompra={resumen.total}
                          onUpdate={(id, patch) => updateMedioPago(id, adaptVentaPatchToIngreso(patch))}
                          onRemove={(rowId) =>
                            removeMedioPago({
                              id: rowId,
                              id_cheque: (form.medios || []).find((x) => x.id === rowId)?.id_cheque,
                            })
                          }
                          onAdd={addMedioPago}
                          showToast={showToast}
                              ...
                              <>
                                {mostrarArchivoActual && (
                                  <div className="mi-uploadFile is-filled">
                                    ...
                                    <div className="mi-uploadFile__meta">
                                      ...
                                      <div className="mi-uploadFile__size">Archivo ya vinculado al ingreso</div>
                                    </div>
                                ...
                                {!mostrarArchivoActual && !archivoNuevo && (
                                  <div className="mi-uploadFile is-empty">
                                    <div className="mi-uploadFile__meta">
                                      ...
                                      <div className="mi-uploadFile__size">No hay comprobante seleccionado
</div>
                                    </div>
                                  </div>
                                )}
                                ...
                                <div className="mi-uploadBar" style={{ marginTop: 12 }}>
                                  ...
                                  <button
                                    ...
                                    disabled={saving}
                                  >
                                    {archivoNuevo || mostrarArchivoActual ? "Reemplazar archivo" : "Seleccionar archivo"}
                                  </button>
                                </div>
                              </>
                            )}
      ...
      {chequeEditor.open && (
        <ModalNuevoCheque
          open={chequeEditor.open}
          onClose={() => setChequeEditor({ open: false, medioRowId: null, tipoCheque: "cheque" })}
          onSave={async (datosCheque) => {
            updateMedioPago(chequeEditor.medioRowId, {
              chequeData: {
                ...datosCheque,
                tipo_cheque: chequeEditor.tipoCheque,
                id_cheque:
                  form.medios.find((x) => x.id === chequeEditor.medioRowId)?.chequeData?.id_cheque || null,
              },
              id_cheque:
                form.medios.find((x) => x.id === chequeEditor.medioRowId)?.id_cheque || null,
            ...
            showToast(
              "exito",
              `${chequeEditor.tipoCheque === "echeq" ? "eCheq" : "Cheque"} actualizado.`,
              2800
            );
          }}
          initialData={form.medios.find((x) => x.id === chequeEditor.medioRowId)?.chequeData || undefined}
          tipoCheque={chequeEditor.tipoCheque}
          dark={dark}
          saving={false}
        />
      )}
