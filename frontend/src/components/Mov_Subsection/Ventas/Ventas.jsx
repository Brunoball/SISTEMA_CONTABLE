...
function buildExportRows(rows) { return (Array.isArray(rows) ? rows : []).map((r) => ({ FECHA: safeText(formatFechaDMY(r?.fecha)), DESCRIPCION: safeText(r?.detalle ?? r?.descripcion ?? r?.concepto), CLIENTE: safeText(r?.cliente), PAGO: safeText(r?.pago_tipo_venta ?? r?.tipo_venta), MEDIO_DE_PAGO: safeText(r?.medio_pago_nombre), TOTAL: Number(r?.monto_total ?? r?.total ?? r?.total_general ?? 0) || 0 })); }
...
export default function Ventas() {
      ...
      return await new Promise((resolve) => {
        const apply = () => {
          ...
          if (append) {
            ...
            rowsRef.current = page; setRows(page); setHasMore(newHasMore); setNextOffset(newNextOffset); if (offset === 0) cacheRef.current.set(cacheKey, { rows: page, hasMore: newHasMore, nextOffset: newNextOffset }); if (rowsReqIdRef.current === myReqId) setLoadingRows(false); prewarmAllComprobantes(page);
          }
  ...
  const handleDateRangeChange = useCallback(async (newRange) => {
    ...
    setDateRange(newRange); cacheRef.current.clear(); skipSearchRef.current = true; liveTokenRef.current = null; if (searchTimerRef.current) clearTimeout(searchTimerRef.current);
  ...
  const columns = useMemo(() => [
    ...
    { key: "pago", label: "PAGO", fr: 1.2, align: "center", render: (r) => { const tv = String(r.pago_tipo_venta ?? r.tipo_venta ?? "").trim(); if (tv) return tv; const id = Number(r.id_tipo_venta ?? 0); return Number.isFinite(id) && id > 0 ? `ID ${id}` : "—"; } },
    { key: "medio_pago_nombre", label: "MEDIO DE PAGO", fr: 1.4, align: "center", render: (r) => safeText(r.medio_pago_nombre) },
  ...
  const exportToTXT = useCallback(() => { const dataToExport = getExportData(); const lines = dataToExport.map((row, index) => [ `REGISTRO ${index + 1}`, `FECHA: ${row.FECHA ?? ""}`, `DESCRIPCION: ${row.DESCRIPCION ?? ""}`, `CLIENTE: ${row.CLIENTE ?? ""}`, `PAGO: ${row.PAGO ?? ""}`, `MEDIO DE PAGO: ${row.MEDIO_DE_PAGO ?? ""}`, `TOTAL: ${row.TOTAL ?? ""}`, "----------------------------------------" ].join("\n")); downloadBlob(lines.join("\n"), `${exportBaseName}.txt`, "text/plain;charset=utf-8;"); }, [getExportData, exportBaseName]);
  ...
  const skelWidths = useMemo(() => ({ fecha: ["44%", "38%", "40%", "36%"], detalle: ["72%", "58%", "66%", "48%"], cliente: ["62%", "54%", "46%", "58%"], pago: ["44%", "34%", "40%", "30%"], medio_pago_nombre: ["52%", "44%", "48%", "36%"], total: ["38%", "30%", "34%", "28%"] }), []);
