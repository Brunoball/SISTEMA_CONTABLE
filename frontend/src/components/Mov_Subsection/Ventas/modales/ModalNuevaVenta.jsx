...
function normalizeLists(lists) {
  ...
  return {clientes:pick("clientes"),detalles:pick("detalles"),medios_pago:Array.isArray(mediosPago)?mediosPago:[],cuentas_corrientes:Array.isArray(cuentas)?cuentas:[],tipos_venta:Array.isArray(tiposVenta)?tiposVenta:[]};
}
...
function getAuthInfo() {
  ...
  let idUsuario=0;
  try { const u=JSON.parse(localStorage.getItem("usuario")||"null"); const cand=u?.idUsuarioMaster??u?.idUsuario??u?.id_usuario??u?.id??u?.user_id??0; if(Number.isFinite(Number(cand)))idUsuario=Number(cand); } catch {}
...
function normalizeArcaSummary(s) {
  ...
  return {cuit:safeStr(x.cuit),razon_social:safeStr(x.razon_social),condicion_iva:safeStr(x.iva||x.condicion_iva),domicilio:safeStr(x.domicilio),doc_tipo:80,doc_nro:safeStr(x.cuit),origen:"arca_cuit"};
}
function normalizeClienteFiscalDb(data) {
  const s=data&&typeof data==="object"?data:{};
  return {id_cliente_fiscal:Number(s.id_cliente_fiscal||0)||null,id_cliente:Number(s.id_cliente||0)||null,doc_tipo:Number(s.doc_tipo||80)||80,doc_nro:safeStr(s.doc_nro),cuit:safeStr(s.cuit),razon_social:safeStr(s.razon_social),condicion_iva:safeStr(s.condicion_iva||s.cond_iva),domicilio:safeStr(s.domicilio),origen:safeStr(s.origen||"manual")};
}
...
/** Detecta si el medio de pago seleccionado es cheque o echeq */
...
/** Detecta si el medio de pago es echeq específicamente */
...
export default function ModalNuevaVenta({open,lists,onClose,onToast,onSaved}) {
  ...
  useEffect(()=>{
    ...
    if(!wasOpen&&open){
      ...
      setConfigFacturacion(null); setOpenResumenFactura(false); setResumenFacturaData(null);
      setOpenChequeModal(false); setSavingCheque(false); setChequeGuardado(null);
      setTimeout(()=>closeBtnRef.current?.focus(),0);
    }
  },[open]);

  /* Limpiar cheque guardado si cambia el medio de pago */
  useEffect(() => {
  ...
  const guardarNuevoCatalogo=useCallback(async()=>{
    ...
    try {
      ...
      if(kind==="clientes"){updateFilter("id_cliente",String(newId));setCliInput(newNombre);setClienteFiscalDb(null);setFiscalArcaData(null);setFiscalCuitInput("");setFiscalError("");}
      setAddUI({open:false,kind:null,rowId:null,text:"",saving:false});
  ...
  useEffect(()=>{
    if(!open)return;
    const direct=Number(filters.id_cliente); const fallbackId=getClienteId(clienteResolvedFromInput);
  ...
  const tipoVentaSelected=useMemo(()=>{const id=Number(filters.id_tipo_venta);if(!Number.isFinite(id)||id<=0)return null;return tiposVentaList.find(x=>Number(x?.id??x?.id_tipo_venta??0)===id)||null;},[filters.id_tipo_venta,tiposVentaList]);
  ...
  const fetchClienteFiscal=useCallback(async idCliente=>{
    ...
    try{const data=await apiGetJson(`${API_GET_CLIENTE_FISCAL}&id_cliente=${id}`);if(data?.existe&&data?.cliente_fiscal){const n=normalizeClienteFiscalDb(data.cliente_fiscal);setClienteFiscalDb(n);setFiscalCuitInput(n.cuit||n.doc_nro||"");return n;}setClienteFiscalDb(null);return null;}
    catch(e){setFiscalError(e?.message||"Error consultando datos fiscales.");return null;}
    finally{setFiscalLoading(false);}
  },[API_GET_CLIENTE_FISCAL]);

  const fetchConfigFacturacion=useCallback(async()=>{const data=await apiGetJson(API_CONFIG_FACTURACION);const cfg=data?.config||data?.data||data||null;if(!cfg)throw new Error("No se pudo obtener config de facturación.");setConfigFacturacion(cfg);return cfg;},[API_CONFIG_FACTURACION]);

  const buscarFiscalEnArcaPorCuit=useCallback(async cuitRaw=>{
    const cuit=onlyDigits(cuitRaw);setFiscalError("");setFiscalArcaData(null);
    if(cuit.length!==11)throw new Error("Ingresá un CUIT válido de 11 dígitos.");
    setFiscalLookupLoading(true);
    try{const data=await apiGetJson(`${API_PADRON_CUIT}&cuit=${cuit}`);const summary=data?.data?.summary??data?.summary??null;if(!summary)throw new Error("ARCA no devolvió datos para ese CUIT.");const norm=normalizeArcaSummary(summary);if(!norm.cuit||!norm.razon_social)throw new Error("ARCA devolvió datos incompletos.");setFiscalArcaData(norm);return norm;}
    catch(e){setFiscalArcaData(null);setFiscalError(e?.message||"Error consultando ARCA.");throw e;}
    finally{setFiscalLookupLoading(false);}
  },[API_PADRON_CUIT]);
  ...
  const guardarClienteFiscal=useCallback(async fiscalSource=>{
    ...
    const fiscal=normalizeClienteFiscalDb(fiscalSource||{});if(!fiscal.cuit||!fiscal.razon_social)throw new Error("Datos fiscales inválidos.");
    const {idUsuario}=getAuthInfo();
    const saved=await apiPostJson(API_SAVE_CLIENTE_FISCAL,{idUsuario,id_cliente:selectedClienteId,doc_tipo:Number(fiscal.doc_tipo||80),doc_nro:fiscal.doc_nro||fiscal.cuit,cuit:fiscal.cuit,razon_social:fiscal.razon_social,condicion_iva:fiscal.condicion_iva,domicilio:fiscal.domicilio,origen:fiscal.origen||"arca_cuit",activo:1});
    if(!saved?.exito||!saved?.cliente_fiscal)throw new Error(saved?.mensaje||"No se pudieron guardar los datos fiscales.");
    const n=normalizeClienteFiscalDb(saved.cliente_fiscal);setClienteFiscalDb(n);setFiscalCuitInput(n.cuit||n.doc_nro||"");return n;
  ...
  const resolveFiscalForFacturacion=useCallback(async()=>{
    ...
    showToast("exito","Datos fiscales obtenidos y guardados correctamente.",2600);return fiscalGuardado;
  ...
  const buildResumenFacturaPayload=useCallback((clienteFiscalResuelto,cfg)=>{
    const items=rowsCalc.filter(r=>Number.isFinite(Number(r.id_detalle))&&Number(r.id_detalle)>0&&Number(r.total||0)>0).map((r,i)=>({id:r.id,codigo:String(i+1),descripcion:safeStr(r.detalleText),cantidad:Number(r.cantidad||0),unidad:"u",precio_unitario:Number(r.precio||0),precio:Number(r.precio||0),bonif_pct:0,impBonif:0,subtotal:Number(r.subtotal||0),ars:Number(r.total||0),iva_pct:Number(r.ivaPct||0),iva_monto:Number(r.ivaMonto||0),total:Number(r.total||0)}));
    ...
    return {id_pago:null,id_sistema:null,labelCliente:selectedClienteNombre||"Cliente",labelSistema:"Nueva venta",cliente_facturacion:{doc_tipo:Number(clienteFiscalResuelto?.doc_tipo||80),doc_nro:safeStr(clienteFiscalResuelto?.doc_nro||clienteFiscalResuelto?.cuit),cuit:safeStr(clienteFiscalResuelto?.cuit),razon_social:safeStr(clienteFiscalResuelto?.razon_social),cond_iva:safeStr(clienteFiscalResuelto?.condicion_iva||clienteFiscalResuelto?.cond_iva),condicion_iva:safeStr(clienteFiscalResuelto?.condicion_iva||clienteFiscalResuelto?.cond_iva),domicilio:safeStr(clienteFiscalResuelto?.domicilio),origen:safeStr(clienteFiscalResuelto?.origen||"arca_cuit")},id_cliente:selectedClienteId||null,id_tipo_venta:Number(filters.id_tipo_venta||0)||null,id_medio_pago:isContado?Number(filters.id_medio_pago||0)||null:null,id_clasificacion:null,fecha_cbte_iso:String(fecha||todayISO()).slice(0,10),vto_pago_iso:plusDaysISOFrom(fecha||todayISO(),10),cbte_tipo:codigoCbte,pto_vta:puntoVenta,items_facturacion:items,total_ars:Number(resumen.total||0),monto:Number(resumen.total||0),importe:Number(resumen.total||0),observaciones:"",emisor_nombre:safeStr(cfg?.razon_social||cfg?.nombre_fantasia||"BALTO"),emisor_domicilio:safeStr(cfg?.domicilio_comercial),cuit_emisor:safeStr(cfg?.cuit),cond_iva_emisor:safeStr(cfg?.condicion_iva),ingresos_brutos_emisor:safeStr(cfg?.ingresos_brutos),fecha_inicio_actividades_emisor:safeStr(cfg?.fecha_inicio_actividades),logo_url:safeStr(cfg?.logo_url)};
  ...
  const guardarVentaBatch=useCallback(async({clienteFiscalResuelto=null,accionFinal="guardar",esFacturadaFinal=false})=>{
    const {idUsuario}=getAuthInfo(); const periodoApi=fechaToYYYYMM(fecha);
    const payloads=rowsCalc.filter(r=>Number.isFinite(Number(r.id_detalle))&&Number(r.id_detalle)>0&&Number(r.total||0)>0).map(r=>({idUsuario,fecha,periodo:periodoApi,id_cliente:selectedClienteId>0?selectedClienteId:null,cliente_nombre:selectedClienteNombre||null,id_tipo_venta:Number(filters.id_tipo_venta),id_medio_pago:isContado?Number(filters.id_medio_pago):null,id_cuenta_corriente:null,id_detalle:Number(r.id_detalle),cantidad:Math.round(Number(r.cantidad)*100)/100,precio:Math.round(Number(r.precio)*100)/100,iva_pct:Math.round(Number(r.ivaPct)*100)/100,subtotal:Math.round(Number(r.subtotal)*100)/100,iva_monto:Math.round(Number(r.ivaMonto)*100)/100,total:Math.round(Number(r.total)*100)/100,monto_total:Math.round(Number(r.total)*100)/100,accion_venta:accionFinal,es_facturada:esFacturadaFinal,cliente_fiscal:esFacturadaFinal?clienteFiscalResuelto:null}));
  ...
  const vincularComprobanteAMovimientosLote=useCallback(async(idsMovimiento,idComprobante)=>{
    ...
    const data=await apiPostJson(API_VINCULAR_COMPROBANTE_LOTE,{id_comprobante:Number(idComprobante),ids_movimiento:idsMovimiento.map(x=>Number(x)).filter(x=>Number.isFinite(x)&&x>0),force:false});
    if(!data?.exito)throw new Error(data?.mensaje||"No se pudo vincular el comprobante al lote.");return data;
  ...
  const abrirResumenFactura=useCallback(async()=>{
    ...
    setSaving(true);
    try{const cf=await resolveFiscalForFacturacion();const cfg=configFacturacion||(await fetchConfigFacturacion());setResumenFacturaData(buildResumenFacturaPayload(cf,cfg));setOpenResumenFactura(true);}
    catch(e){showToast("error",e?.message||"No se pudo preparar la factura.",4500);}
  ...
  const onClickFacturar=useCallback(async()=>{
    setAccionContado("facturar");setFiscalError("");
    if(!selectedClienteId){showToast("advertencia","Seleccioná un cliente antes de facturar.",3200);return;}
    try{
      setSaving(true);const fiscal=clienteFiscalDb||(await fetchClienteFiscal(selectedClienteId));
    ...
    }catch(e){showToast("error",e?.message||"No se pudo iniciar la facturación.",4200);}
    finally{setSaving(false);}
