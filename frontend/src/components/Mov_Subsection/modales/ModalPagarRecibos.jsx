...
export default function ModalPagarRecibos({
  ...
  const toggleOne = (id) => {
    setSelectedIds((prev) => {
      ...
      if (next.size === deudasOrdenadas.length && deudasOrdenadas.length > 0)
        setPagaTodo(true);
  ...
  const toggleAll = () => {
    const all = deudasOrdenadas
      .map((r) => Number(r?.id_movimiento || 0))
      .filter(Boolean);
    setSelectedIds(() => {
      const next = new Set();
      const shouldSelectAll = selectedIds.size !== all.length;
        ...
        <div className="mi-modal__body mpr-body">
          <div className="mpr-content">
            ...
            <div className="mpr-actions">
              <button
                type="button"
                className="mov-btn mov-btn--ghost mpr-btnWide"
                onClick={toggleAll}
            ...
            {/* Nota (opcional) - si querés usarla, descomentá este bloque */}
            {/* 
            ...
            */}
            ...
            <div className="mpr-tableWrap">
              <div className="mpr-tableTitle">Deudas pendientes</div>
                  ...
                  {deudasOrdenadas.map((r, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <label
                            className={`mpr-checkWrap ${(!id || loading) ? "is-disabled" : ""}`}
                          >
                            <input
