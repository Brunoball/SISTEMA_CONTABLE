...
function getCategoriaVisualByStock(stockValue) {
  ...
  switch (key) {
    case "sin_stock": return { key, label: "Sin stock",   className: "stock-badge stock-badge--sin"   };
    case "bajo":      return { key, label: "Stock bajo",  className: "stock-badge stock-badge--bajo"  };
    case "medio":     return { key, label: "Stock medio", className: "stock-badge stock-badge--medio" };
    case "alto":      return { key, label: "Stock alto",  className: "stock-badge stock-badge--alto"  };
    default:          return { key: "", label: "Sin categoría", className: "stock-badge" };
  }
}
...
export default function Stock() {
  ...
  const fetchCategoriasGlobales = useCallback(async () => {
    ...
    const cats = Array.isArray(data?.listas?.stock_categorias) ? data.listas.stock_categorias : [];
    setCategoriasGlobales(cats.map((cat) => ({
  ...
  const recargarTodo = useCallback(async ({ mostrarToastExito = false } = {}) => {
    setLoading(true);
    setError("");
    try {
      await Promise.all([fetchCategoriasGlobales(), fetchTodosLosProductos()]);
      if (mostrarToastExito) mostrarToast("exito", "Stock actualizado correctamente.");
    } catch (err) {
  ...
  useEffect(() => { recargarTodo(); }, [recargarTodo]);
  ...
  const resumen = useMemo(() => {
    const base = { total_productos: allProductos.length, total_unidades: 0, sin_stock: 0, bajo: 0, medio: 0, alto: 0 };
  ...
  const productosFiltrados = useMemo(() => {
    ...
    return allProductos.filter((prod) => {
      ...
      const matchNivel =
        nivelSeleccionado === "todas" || getCategoriaStockKey(prod?.stock) === nivelSeleccionado;
      return matchBusqueda && matchCategoria && matchNivel;
    });
  }, [allProductos, busqueda, categoriaSeleccionada, nivelSeleccionado]);

  const total        = productosFiltrados.length;
  const totalPaginas = useMemo(() => Math.max(1, Math.ceil(total / ITEMS_POR_PAGINA)), [total]);

  useEffect(() => { if (paginaActual > totalPaginas) setPaginaActual(1); }, [paginaActual, totalPaginas]);
  ...
  const miniCards = [
    { title: "Total",     value: resumen.total_productos, icon: faBoxesStacked,       className: "stock-summaryCard--blue",   onClick: () => { setNivelSeleccionado("todas");     setPaginaActual(1); } },
    { title: "Sin stock", value: resumen.sin_stock,       icon: faTriangleExclamation, className: "stock-summaryCard--red",   onClick: () => { setNivelSeleccionado("sin_stock"); setPaginaActual(1); } },
    { title: "Bajo",      value: resumen.bajo,            icon: faLayerGroup,          className: "stock-summaryCard--orange", onClick: () => { setNivelSeleccionado("bajo");    setPaginaActual(1); } },
    { title: "Medio",     value: resumen.medio,           icon: faWarehouse,           className: "stock-summaryCard--yellow", onClick: () => { setNivelSeleccionado("medio");   setPaginaActual(1); } },
    { title: "Alto",      value: resumen.alto,            icon: faBoxOpen,             className: "stock-summaryCard--green",  onClick: () => { setNivelSeleccionado("alto");    setPaginaActual(1); } },
    { title: "Uds.",      value: resumen.total_unidades,  icon: faWarehouse,           className: "stock-summaryCard--dark",   onClick: null, static: true },
  ];
  ...
  return (
    <>
      <div className="mov-page stock-page">
        ...
        <section className="mov-card mov-card--table">
          ...
          <div className="mov-card__head stock-card__head">
            ...
            {/* Fila 2: mini-cards + buscador + select */}
            <div className="stock-head__bottom">


              <div className="stock-miniCards">
                {miniCards.map((card) =>
                  card.static ? (
                    <div key={card.title} className={`stock-miniCard stock-miniCard--static ${card.className}`}>
                      <FontAwesomeIcon icon={card.icon} />
                      <span className="stock-miniCard__value">{loading ? "…" : card.value}</span>
                      <span className="stock-miniCard__label">{card.title}</span>
                    </div>
                  ) : (
                    <button
                      key={card.title}
                      type="button"
                      className={`stock-miniCard ${card.className}`}
                      onClick={card.onClick}
                      disabled={loading}
                      title={`Filtrar: ${card.title}`}
                    >
                      <FontAwesomeIcon icon={card.icon} />
                      <span className="stock-miniCard__value">{loading ? "…" : card.value}</span>
                      <span className="stock-miniCard__label">{card.title}</span>
            ...
            <div className="mov-gridBody mov-gridBody--relative">
              {loading ? (
                ...
                <div className="cc-emptyState">
                  ...
                  <div className="cc-emptyText">No se encontraron productos con los filtros seleccionados.</div>
                </div>
              ) : (
                  ...
                  return (
                    <div key={prod.id} className="mov-gridTable mov-gridTable--row" style={{ gridTemplateColumns: GRID_COLS }} role="row">
                      ...
                      <div className="mov-gridCell is-center" role="cell" data-label="CATEGORÍA">
                        <span className="mov-ellipsissss">{prod.categoria_nombre || "Sin categoría"}</span>
                      </div>
                      <div className="mov-gridCell is-center" role="cell" data-label="CANTIDAD">
                        <span className={Number(prod.stock || 0) <= 0 ? "mov-chip mov-chip--warn" : "mov-chip mov-chip--ok"}>
                          {Number(prod.stock || 0)}
                        </span>
                      </div>
      ...
      <ModalCategoriasStock
        ...
        onActualizado={async () => { await recargarTodo({ mostrarToastExito: false }); }}
      />

      {toast && (
        <Toast key={toast.id} tipo={toast.tipo} mensaje={toast.mensaje} duracion={toast.duracion} onClose={cerrarToast} />
      )}
    </>
  );
}