...
export default function StockProveedores() {
  ...
  const resetForm = useCallback(() => {
    setModo("crear");
    setEditandoId(null);
    setForm({ nombre: "", activo: pestana === "inactivos" ? 0 : 1 });
  }, [pestana]);

  const cargarProveedores = useCallback(async (tabActual = pestana) => {
    setLoading(true);
    try {
  ...
  useEffect(() => {
    ...
    resetForm();
  }, [pestana, cargarProveedores, resetForm]);
  ...
  const ejecutarAccionModal = useCallback(async () => {
    ...
    try {
      ...
      if (type === "baja") {
        ...
        successFallback = "Proveedor dado de baja.";
        recargarTab = "activos";
      } else if (type === "alta") {
        action = "stock_proveedor_dar_alta";
        successFallback = "Proveedor dado de alta.";
        recargarTab = "inactivos";
      } else {
        action = "stock_proveedor_eliminar";
        successFallback = "Proveedor eliminado.";
        recargarTab = pestana;
      }
  ...
  }, [modalAccion, pestana, cargarProveedores, editandoId, resetForm, cerrarModalAccion, mostrarToast]);
  ...
  const modalConfig = useMemo(() => {
    ...
    if (modalAccion.type === "baja") {
      return {
        ...
        warning: "Pasará a inactivos.",
        confirmLabel: "Dar de baja",
        cancelLabel: "Cancelar",
        variant: "danger",
        details: [{ label: "Proveedor", value: nombre }],
      };
    }
        ...
        <div className="mi-cr-grid" style={{ display: "grid", gridTemplateColumns: "280px 1fr", gap: 20, alignItems: "start" }}>
            ...
            <div className="mi-cr-filters__body">
              ...
              <div className="mi-cr-filters__actions" style={{ flexDirection: "column" }}>
                <button type="button" className="mit-btn mit-btn--solid mit-btn--block" onClick={handleGuardar} disabled={saving}>
                  ...
                  {saving ? "Guardando..." : modo === "crear" ? "Crear proveedor" : "Guardar cambios"}
                </button>
          ...
          <section className="mi-cr-table mov-card" style={{ padding: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 12 }}>
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: "var(--nv-text)" }}>
                  <FontAwesomeIcon icon={faTruckField} style={{ marginRight: 8, opacity: 0.7 }} />
                  Stock · Proveedores
                </div>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button type="button" className={`mit-btn ${pestana === "activos" ? "mit-btn--solid" : "mit-btn--ghost"}`} onClick={() => setPestana("activos")} disabled={loading || saving}>
                  Activos
                </button>
                <button type="button" className={`mit-btn ${pestana === "inactivos" ? "mit-btn--solid" : "mit-btn--ghost"}`} onClick={() => setPestana("inactivos")} disabled={loading || saving}>
                  Inactivos
                </button>
              </div>
            </div>
            ...
            <div className="mi-cr-table__rows mi-cr-table__rows--mcs">
              {loading ? (
                <EmptyState icon={faArrowRotateRight} spin text="Cargando proveedores..." />
              ) : proveedoresOrdenados.length === 0 ? (
                <EmptyState icon={faTruckField} text={pestana === "activos" ? "No hay proveedores activos." : "No hay proveedores inactivos."} />
              ) : (
                proveedoresOrdenados.map((row) => {
                  const activo = Number(row?.activo ?? 1) === 1;
                  const isEditing = getProveedorId(row) === Number(editandoId || 0) && modo === "editar";
                  const bloqueado = accionandoId === getProveedorId(row) || saving;

                  return (
                    <div key={getProveedorId(row)} className={["mi-cr-row", "mi-cr-grid-clientes-stock", isEditing ? "mi-cr-row--editing" : ""].join(" ").trim()}>
                      <div className="mi-cr-cell mi-cr-cell--ellipsis" style={{ paddingLeft: 10, minWidth: 0 }}>
                        <span title={row?.nombre || "—"} style={{ display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontWeight: 600, fontSize: 13, color: "var(--nv-text)" }}>
                          {row?.nombre || "—"}
                        </span>
                      </div>
                      ...
                      <div className="mi-cr-cell mi-cr-cell--center">
                        <div style={{ display: "flex", gap: 6, padding: 5, flexWrap: "wrap", justifyContent: "center" }}>
                          {activo ? (
                            <>
                              <button type="button" className="nv-foot-btn nv-foot-btn--sm" onClick={() => iniciarEdicion(row)} disabled={bloqueado} title="Editar">
                                <FontAwesomeIcon icon={faPenToSquare} />
                              </button>
                              <button type="button" className="nv-foot-btn nv-foot-btn--sm" onClick={() => abrirModalAccion("baja", row)} disabled={bloqueado} title="Dar de baja">
                                <FontAwesomeIcon icon={faUserSlash} />
                              </button>
                            </>
                          ) : (
                            <button type="button" className="nv-foot-btn nv-foot-btn--sm" onClick={() => abrirModalAccion("alta", row)} disabled={bloqueado} title="Dar de alta">
                              <FontAwesomeIcon icon={faUserCheck} />
                            </button>
                          )}

                          <button type="button" className="nv-foot-btn nv-foot-btn--sm nv-foot-btn--danger" onClick={() => abrirModalAccion("eliminar", row)} disabled={bloqueado} title="Eliminar">
